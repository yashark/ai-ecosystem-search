...
def get_weekly_intel(conn):
    """Get this week's intel summary for the dashboard card."""
    news_7d = query(conn, "SELECT COUNT(*) FROM news_mentions WHERE scanned_at >= date('now', '-7 days')")[0][0]
    new_startups_7d = query(conn, f"SELECT COUNT(*) FROM startups WHERE processed_at >= date('now', '-7 days') AND Status != 'removed'")[0][0]
    # Top headlines
    headlines = query(conn, """
        SELECT nm.headline, nm.source_name, nm.event_category,
               COALESCE(s.company_name, inv.investor_name, '') as entity
        ...
        WHERE nm.scanned_at >= date('now', '-7 days')
              AND nm.headline IS NOT NULL AND nm.headline != ''
        ORDER BY nm.scanned_at DESC LIMIT 6
    """)
    items = []
    for r in headlines:
        items.append({
    ...
    cats = query(conn, """
        ...
        FROM news_mentions WHERE scanned_at >= date('now', '-7 days')
        GROUP BY event_category ORDER BY cnt DESC LIMIT 5
    """)
    cat_breakdown = [{"cat": r[0] or "uncategorized", "cnt": r[1]} for r in cats]
    return {
        "news_count": news_7d,
...
def get_kpis(conn, sectors, cities, years, valid_deal_ids):
    ...
    avg_conf = query(conn, f"SELECT AVG(data_confidence) FROM startups WHERE {BASE_FILTER}")[0][0]
    total_investors = query(conn, "SELECT COUNT(*) FROM investors")[0][0]
    return {
        "total": total,
...
def generate_html(data):
    """Load HTML template and inject data constants + KPI values."""
    ...
    # Build JS data block
    data_block = "\n".join([
        js_const("SECTORS", data["sectors"]),
        js_const("SUBCATEGORIES", data["subcategories"]),
        js_const("CITIES", data["cities"]),
        js_const("YEARS", data["years"]),
        js_const("BMODELS", data["bmodels"]),
        js_const("AI_BUCKETS", data["ai_buckets"]),
        js_const("AI_PROXIMITY_DIST", data["ai_proximity_dist"]),
        js_const("CLOUD_RELEVANCE", data["cloud_relevance"]),
        js_const("DATA_CONFIDENCE", data["data_confidence"]),
        js_const("TOP_INVESTORS", data["top_investors"]),
        js_const("ROUND_TYPES", data["round_types"]),
        js_const("DEAL_TIMELINE", data["deal_timeline"]),
        js_const("TECH_LIST", data["tech_list"]),
        js_const("ALL_STARTUPS", data["all_startups"]),
    ])
    ...
    # Inject KPI values
    html = html.replace("<!--KPI_TOTAL-->", f"{k['total']:,}")
    html = html.replace("<!--KPI_AVG_AI-->", str(k["avg_ai"]))
    html = html.replace("<!--KPI_CORE_AI-->", f"{k['core_ai']:,}")
    html = html.replace("<!--KPI_INVESTORS-->", f"{k['investor_count']:,}")
    html = html.replace("<!--KPI_DEALS-->", f"{k['total_deals']:,}")
    html = html.replace("<!--KPI_CITIES-->", str(k["city_count"]))
    html = html.replace("<!--KPI_TOP_CITY-->", k["top_city"])
    html = html.replace("<!--KPI_TOP_CITY_PCT-->", str(k["top_city_pct"]))
    html = html.replace("<!--KPI_RECENT-->", f"{k['recent']:,}")
    html = html.replace("<!--KPI_RECENT_PCT-->", str(k["recent_pct"]))
    html = html.replace("<!--KPI_PRODUCT_PCT-->", str(k["product_pct"]))
    html = html.replace("<!--KPI_AVG_CONF-->", str(k["avg_conf"]))
    html = html.replace("<!--KPI_INVESTOR_COUNT-->", f"{k['total_investors']:,}")
    html = html.replace("<!--DATA_DATE-->", datetime.now().strftime("Data as of %B %Y"))

    # Inject weekly intel
    wi = data["weekly_intel"]
    html = html.replace("<!--INTEL_NEWS_COUNT-->", str(wi["news_count"]))
    html = html.replace("<!--INTEL_NEW_STARTUPS-->", str(wi["new_startups"]))

    # Build intel headlines HTML
    headlines_html = ""
    cat_colors = {"funding": "var(--green)", "ecosystem": "var(--blue)", "product": "var(--purple)",
                  "partnership": "var(--amber)", "education": "var(--cyan)"}
    for item in wi["headlines"]:
        cat = item["category"]
        color = cat_colors.get(cat, "var(--muted)")
        cat_label = cat if cat else "news"
        entity_tag = f' <span style="color:var(--dim)">&middot; {item["entity"]}</span>' if item["entity"] else ""
        headlines_html += (
            f'<div class="intel-item">'
            f'<span class="intel-cat" style="border-color:{color};color:{color}">{cat_label}</span>'
            f'<span class="intel-headline">{item["headline"]}</span>'
            f'{entity_tag}'
            f'<span class="intel-src">{item["source"]}</span>'
            f'</div>\n'
        )
    html = html.replace("<!--INTEL_HEADLINES-->", headlines_html)

    # Build category breakdown
    cats_html = ""
    for c in wi["categories"]:
        cat = c["cat"]
        color = cat_colors.get(cat, "var(--dim)")
        cats_html += (
            f'<div class="intel-cat-row">'
            f'<span class="intel-cat-name" style="color:{color}">{cat}</span>'
            f'<span class="intel-cat-cnt">{c["cnt"]}</span>'
            f'</div>\n'
        )
    html = html.replace("<!--INTEL_CATEGORIES-->", cats_html)
...
def main():
    ...
    weekly_intel = get_weekly_intel(conn)
    conn.close()
    data = {
